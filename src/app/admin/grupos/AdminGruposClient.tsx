...
function paletteFor(index: number) { return PALETTES[index % PALETTES.length] }

export default function AdminGruposClient({
  ...
  async function openLibros(grupo: Grupo) {
    ...
    const res = await fetch(`/api/admin/libro-grupos?grupo_id=${grupo.id}`)
  ...
  async function handleResetPasswords(grupo: Grupo) {
    ...
    const res = await fetch(`/api/admin/grupos/${grupo.id}/reset-passwords`, { method: 'POST' })
    const data = await res.json()
  ...
  function getAlumnosDelGrupo(grupoId: string) {
    return alumnos.filter(a => a.grupo_alumnos?.some(ga => ga.grupo_id === grupoId))
  }

  function getAlumnosDisponibles(grupoId: string) {
    return alumnos.filter(a => !a.grupo_alumnos?.some(ga => ga.grupo_id === grupoId) && a.activo)
  }
  ...
  function handleAlumnoAgregado(grupoId: string, alumnoId: string) {
    setAlumnos(prev => prev.map(a => {
      ...
      return { ...a, grupo_alumnos: [{ grupo_id: grupoId, grupo: grupoObj ? { id: grupoId, nombre: grupoObj.nombre } : null }] }
    }))
  ...
  function handleAlumnoQuitado(grupoId: string, alumnoId: string) {
    setAlumnos(prev => prev.map(a => a.id === alumnoId ? { ...a, grupo_alumnos: [] } : a))
    setGrupos(prev => prev.map(g => {
    ...
    <>
      ...
      {deleteConfirm && (
        <Confirm
          ...
          message={`¿Eliminar el grupo "${deleteConfirm.nombre}"?`}
      ...
      {deleteError && (
        <Modal open onClose={() => setDeleteError(null)} title="No se puede eliminar" size="sm">
          <div className="space-y-4">
            ...
            <p className="text-sm text-slate-600">Puedes desactivar el grupo para que no sea visible.</p>
        ...
        {grupos.length === 0 && (
          <div className="card p-12 text-center space-y-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 mt-1">Crea el primer grupo y asígnale un catequista y alumnos.</p>
            </div>
          </div>
        )}
            ...
            <div key={grupo.id} className={cn('card overflow-hidden', !grupo.activo && 'opacity-60')}>
              ...
              <div className="p-4 space-y-3">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {!grupo.activo && (
                    <span className="px-2.5 py-1 bg-red-100 text-red-600 rounded-full text-xs font-semibold">Inactivo</span>
                  )}
                </div>
        ...
        <div className="space-y-1.5">
          ...
          {catequistas.length === 0 ? (
            ...
            <select className="input" value={catequistaId} onChange={e => setCatequistaId(e.target.value)}>
              ...
              {catequistas.map(c => <option key={c.id} value={c.id}>{nombreCompleto(c)}</option>)}
            </select>
          )}
        </div>
        <div className="flex gap-2 pt-1">
          <button onClick={onClose} className="btn-ghost flex-1">Cancelar</button>
          <button onClick={handleCreate} disabled={loading || !nombre.trim()} className="btn-primary flex-1">
    ...
    <Modal open onClose={onClose} title="Editar grupo">
      <div className="space-y-4">
        ...
        <div className="space-y-1.5">
          <label className="text-sm font-semibold text-slate-700">Catequista responsable</label>
          <select className="input" value={catequistaId} onChange={e => setCatequistaId(e.target.value)}>
            <option value="">Sin catequista</option>
            {catequistas.map(c => <option key={c.id} value={c.id}>{nombreCompleto(c)}</option>)}
          </select>
        </div>
        <div className="flex gap-2 pt-1">
...
function GestionarAlumnosModal({
  grupo, paletteIndex, alumnosDelGrupo, alumnosDisponibles,
  onClose, onAgregado, onQuitado, onToast,
}: {
  grupo: Grupo
  paletteIndex: number
  ...
  async function handleAgregar(alumno: Alumno, confirmado = false) {
    ...
    if (grupoActual && !confirmado) { setConfirmMover(alumno); return }
    setLoadingId(alumno.id)
    const res = await fetch(`/api/admin/grupos/${grupo.id}/alumnos`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ alumnoId: alumno.id }),
    })
    const data = await res.json()
    setLoadingId(null); setConfirmMover(null)
              ...
              <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                {alumnosDelGrupo.map(alumno => (
                  <div key={alumno.id} className="flex items-center gap-3 px-3 py-2.5 bg-slate-50 rounded-xl">
                    <Image src={avatarUrl(alumno.avatar_id)} alt={nombreCompleto(alumno)} width={36} height={36} className="rounded-xl flex-shrink-0" />
            ...
            {disponiblesFiltrados.length === 0 ? (
              <div className="px-3 py-5 text-center">
                <p className="text-sm text-slate-400">
                  {busqueda ? 'No se encontró ese alumno.' : 'No hay alumnos disponibles.'}
                </p>
              </div>
            ) : (
                    ...
                    <div key={alumno.id} className="flex items-center gap-3 px-3 py-2.5 border border-slate-200 rounded-xl hover:border-brand-200 hover:bg-brand-50/30 transition-colors">
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          grupoActual ? 'bg-amber-100 text-amber-700 hover:bg-amber-200' : 'bg-brand-100 text-brand-700 hover:bg-brand-200'
                        )}
                      >
    ...
    <Modal open onClose={onClose} title={`Libros — ${grupo.nombre}`}>
      <div className="space-y-3">
        <p className="text-sm text-slate-500">
          ...
          {asignados > 0 && <span className="ml-1 font-semibold text-brand-600">{asignados} asignado{asignados !== 1 ? 's' : ''}.</span>}
        </p>
        {loading ? (
