...
export default function SeguridadClient({ initialPasswords }: Props) {
  ...
  const [loading, setLoading] = useState(initialPasswords.length === 0)
  ...
  // Fetch fresh data on mount in case SSR returned stale/empty
  useEffect(() => {
    fetch('/api/admin/passwords')
      .then(r => r.json())
      .then(data => {
        if (Array.isArray(data.passwords)) setPasswords(data.passwords)
      })
      .catch(() => {})
          ...
          {showForm ? (
            <div className="card p-5 space-y-4">
              ...
              <div className="flex gap-2">
                <button onClick={handleAdd} disabled={saving || !password} className="btn-primary flex-1">
